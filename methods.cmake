...
function(generate_action_hash __OUTPUT)
	...
	if (NOT "${__ARGS_DEPENDENT_FILES}" STREQUAL "")
		foreach (__FILE IN LISTS __ARGS_DEPENDENT_FILES)
			...
			if (EXISTS "${__NORMILIZED_FILE_PATH}")
				file(MD5 "${__NORMILIZED_FILE_PATH}" __FILE_HASH)
			else()
				set(__FILE_HASH "0")
			endif()

			list(APPEND __TOTAL_HASH_STRING "${__NORMILIZED_FILE_PATH}${__FILE_HASH}")
		endforeach()
	endif()
...
function(add_python_generator_command __MODULE __FUNCTION)
	...
	set(__ONE_VALUE_ARGS 
		...
		COMMENT # Additional comment for the command to display
	)
	set(__MULTI_VALUE_ARGS 
		...
		BUILD_DEPENDENCIES # Source files which will be added as sources for command, but not provided as sources to python generator method
	...
	set(__DEPENDS ${__ARGS_SOURCE_FILES} ${__ARGS_BUILD_DEPENDENCIES})
	...
	# if our command is dependent on any generator expression, we will simply exit from this command then generator expression returns false.
...
function(execute_python_method __MODULE __FUNCTION)
	...
	execute_process(
		COMMAND "${PYTHON_EXECUTABLE}" "-c" "${__METHOD_CALL}"
		WORKING_DIRECTORY "${__ARGS_WORKING_DIR}"
		${__ADDITIONAL_ARGS}
	)
...
# Checks that all required dependencies for specified module are met (e.g. all required modules enabled).
function(check_module_dependencies __OUTPUT __MODULE_ID)
	assert_if_empty(__OUTPUT __MODULE_ID)

	set(__FAILED_DEPENDENCIES "")
	foreach (__DEPENDENCY_MODULE_ID IN ITEMS ${${__MODULE_ID}_REQUIRED_DEPENDENCIES})
		if (NOT godot_module_${__DEPENDENCY_MODULE_ID}_enabled)
			list(APPEND __FAILED_DEPENDENCIES "${__DEPENDENCY_MODULE_ID}")
		endif()
	endforeach()

	if (NOT "${__FAILED_DEPENDENCIES}" STREQUAL "")
		message(WARNING "Disabling '${__MODULE_ID}' module as the following dependencies are not satisfied: ${__FAILED_DEPENDENCIES}.")
		set("${__OUTPUT}" FALSE PARENT_SCOPE)
	else()
		set("${__OUTPUT}" TRUE PARENT_SCOPE)
	endif()
...
# Allows to specify link dependencies between targets (if for example some static library should go after another).
# Essentially this is target_link_libraries with PRIVATE specifier, but also with some additional checks.
function(add_link_dependencies __NAME)
...
# You can specify one library with 'PARENT_LIB' option, to which created library will be automatically linked privately (with PRIVATE keyword),
...
# Also you can specify any number of other libraries, which will be linked to our created library privately.
# You can specify output directory and output name for the library (but this will only work if your library STATIC).
...
# Also you can specify libraries and environments which properties you want to clone into your library with 'LIBS_CLONE' and 'ENVS_CLONE' respectively.
...
function(add_lib __NAME)
	...
	set(__ONE_VALUE_ARGS
		...
		PARENT_ENV # other possibility is to link against environment
	...
	if(NOT __ARGS_OBJECT)
		...
		foreach(__COMP ${__ARGS_COMPONENTS})
			if (TARGET "${__COMP}")
				...
				if (NOT __TARGET_TYPE MATCHES "(OBJECT_LIBRARY|INTERFACE_LIBRARY)")
					message(WARNING "Static library ${__NAME} can't have non OBJECT or non INTERFACE library \"${__COMP}\" as a component. This component will be removed from components list. If you still want to define link dependency of this type, manually call add_link_dependencies.")
		...
		# (OBJECT libraries are loose their objects if they are linked one to another).
		...
		foreach(__COMP ${__ARGS_COMPONENTS})
			if (TARGET "${__COMP}")
				...
				if (NOT __TARGET_TYPE STREQUAL "INTERFACE_LIBRARY")
					message(WARNING "Object library ${__NAME} can't have non INTERFACE library \"${__COMP}\" as a component. This component will be removed from components list.")
					list(REMOVE_ITEM __ARGS_COMPONENTS "${__COMP}")
				endif()
			endif()
		endforeach()
	...
	if(NOT "${__ARGS_DEPENDENCIES}" STREQUAL "")
		add_dependencies("${__NAME}" ${__ARGS_DEPENDENCIES})
	endif()
...
function(target_property __MOD __TARGET __PROPERTY __VALUE)
	...
	elseif (__MOD STREQUAL "APPEND_LIST")
		...
		if (NOT "${__OLD_PROPERTY_VALUE}" STREQUAL "__OLD_PROPERTY_VALUE-NOTFOUND")
			...
			set_target_properties("${__TARGET}" PROPERTIES "${__PROPERTY}" "${__OLD_PROPERTY_VALUE}")
		else()
			set_target_properties("${__TARGET}" PROPERTIES "${__PROPERTY}" "${__VALUE}")
		endif()
...
# because I don't want to include an additional module for the platform I don't use,
# and it's enough for now.
function(target_link_js_libraries __TARGET __SCOPE)
	assert_if_empty(__TARGET __SCOPE)

	foreach (__JS_FILE ${ARGN})
		assert_if_empty(__JS_FILE)

		normilize_path(__NORMILIZED_JS_FILE "${__JS_FILE}" ABSOLUTE)

		if (__SCOPE MATCHES "(PUBLIC|INTERFACE)")
			get_target_property(__INTERFACE_LINKDEPS "${__TARGET}" INTERFACE_LINK_DEPENDS)
			if (__INTERFACE_LINKDEPS STREQUAL "__INTERFACE_LINKDEPS-NOTFOUND")
				set(__INTERFACE_LINKDEPS "")
			endif()

			list(APPEND __INTERFACE_LINKDEPS "${__NORMILIZED_JS_FILE}")
			set_target_properties("${__TARGET}" PROPERTIES INTERFACE_LINK_DEPENDS "${__INTERFACE_LINKDEPS}")
		endif()

		if (__SCOPE MATCHES "(PUBLIC|PRIVATE)")
			get_target_property(__LINKDEPS "${__TARGET}" LINK_DEPENDS)
			if (__LINKDEPS STREQUAL "__LINKDEPS-NOTFOUND")
				set(__LINKDEPS "")
			endif()

			list(APPEND __LINKDEPS "${__NORMILIZED_JS_FILE}")
			set_target_properties("${__TARGET}" PROPERTIES LINK_DEPENDS "${__LINKDEPS}")
		endif()

		target_link_libraries("${__TARGET}" "${__SCOPE}" "--js-library \"${__NORMILIZED_JS_FILE}\"")
	endforeach()
endfunction()
